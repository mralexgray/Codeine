...
+ (NSString *)stringForDataSizeWithBytes:(uint64_t)bytes {
  return [self stringForDataSizeWithBytes:bytes
                                     unit:CEStringDataSizeTypeAuto
                         numberOfDecimals:1];
}

+ (NSString *)stringForDataSizeWithBytes:(uint64_t)bytes
                        numberOfDecimals:(NSUInteger)decimals {
  return [self stringForDataSizeWithBytes:bytes
                                     unit:CEStringDataSizeTypeAuto
...
+ (NSString *)stringForDataSizeWithBytes:(uint64_t)bytes
                                    unit:(CEStringDataSizeType)unit {
...
+ (NSString *)stringForDataSizeWithBytes:(uint64_t)bytes
  ...
  if (unit == CEStringDataSizeTypeAuto) {
    if (bytes >= (uint64_t)1000 * (uint64_t)1000 * (uint64_t)1000 *
                     (uint64_t)1000 * (uint64_t)1000 * (uint64_t)1000 *
  ...
  if (unit == CEStringDataSizeTypeBytes) {
    unitDefaultName = @"B";
    unitKey = [NSString stringWithFormat:@"%s", __NSSTRING_CE_STRINGIFY(
                                                    CEStringDataSizeTypeBytes)];
    number = @(bytes);
  ...
  case CEStringDataSizeTypeKiloBytes:
    ...
    unitDefaultName = @"KB";
    unitKey =
        [NSString stringWithFormat:@"%s", __NSSTRING_CE_STRINGIFY(
                                              CEStringDataSizeTypeKiloBytes)];
  ...
  case CEStringDataSizeTypeKibiBytes:
    ...
    unitKey =
        (unitKey == nil)
            ? [NSString
                  stringWithFormat:@"%s", __NSSTRING_CE_STRINGIFY(
                                              CEStringDataSizeTypeKibiBytes)]
            : unitKey;
  ...
  case CEStringDataSizeTypePebiBytes:
    ...
    unitKey =
        (unitKey == nil)
            ? [NSString
                  stringWithFormat:@"%s", __NSSTRING_CE_STRINGIFY(
                                              CEStringDataSizeTypePebiBytes)]
            : unitKey;
    doubleValue =
        (((((CGFloat)bytes / divider) / divider) / divider) / divider) /
        divider;
  ...
  case CEStringDataSizeTypeZettaBytes:
    ...
    unitKey =
        [NSString stringWithFormat:@"%s", __NSSTRING_CE_STRINGIFY(
                                              CEStringDataSizeTypeZettaBytes)];
  ...
  case CEStringDataSizeTypeYottaBytes:
    ...
    unitKey =
        [NSString stringWithFormat:@"%s", __NSSTRING_CE_STRINGIFY(
                                              CEStringDataSizeTypeYottaBytes)];
