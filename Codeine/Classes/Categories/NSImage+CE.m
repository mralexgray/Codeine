...
- (NSImage *)imageWithSize:(CGFloat)size {
  ...
  [self drawInRect:NSMakeRect((CGFloat)0, (CGFloat)0, size, size)
          fromRect:NSMakeRect((CGFloat)0, (CGFloat)0, self.size.width,
                              self.size.height)
...
- (NSImage *)grayscaleImage {
  ...
  context = CGBitmapContextCreate(
      pixels, width, height, 8, width * sizeof(__RGBPixel), colorSpace,
      kCGBitmapByteOrder32Little | kCGImageAlphaPremultipliedLast);

  CGContextDrawImage(
      context,
      CGRectMake((CGFloat)0, (CGFloat)0, (CGFloat)width, (CGFloat)height),
      [self CGImageForProposedRect:NULL
