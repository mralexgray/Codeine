...
- (NSString *)pathOfDirectory:(NSSearchPathDirectory)directory
  ...
  if (create == YES) {
    status = [self createDirectoryAtPath:path
...
- (NSString *)applicationSupportDirectory {
  ...
  error = nil;
  appName =
      [BUNDLE objectForInfoDictionaryKey:NSBundleInfoKeyCFBundleExecutable];
  path = [self pathOfDirectory:NSApplicationSupportDirectory
                      inDomain:NSUserDomainMask
               byAppendingPath:appName
             createIfNecessary:YES];

  if (path == nil || path.length == 0 ||
      [FILE_MANAGER fileExistsAtPath:path isDirectory:&isDir] == NO ||
      isDir == NO) {
    return nil;
  }
