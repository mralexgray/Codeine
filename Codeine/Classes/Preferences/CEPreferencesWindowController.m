...
- (void)awakeFromNib {
  ...
  for (item in _toolbar.items) {
    if ([[item itemIdentifier] isEqualToString:__generalOptionsItemIdentifier]) {
      [item setImage:[[CESystemIconsHelper sharedInstance] iconNamed:CESystemIconGeneral]];
    } else if ([[item itemIdentifier] isEqualToString:__editorOptionsItemIdentifier]) {
      [item setImage:[[CESystemIconsHelper sharedInstance] iconNamed:CESystemIconClippingText]];
    } else if ([[item itemIdentifier] isEqualToString:__compilerOptionsItemIdentifier]) {
      [item setImage:[[CESystemIconsHelper sharedInstance] iconNamed:CESystemIconToolbarAdvanced]];
    } else if ([[item itemIdentifier] isEqualToString:__linkerOptionsItemIdentifier]) {
      [item setImage:[[CESystemIconsHelper sharedInstance] iconNamed:CESystemIconKEXT]];
    } else if ([[item itemIdentifier] isEqualToString:__fontsAndColorsOptionsIdentifier]) {
      [item setImage:[[CESystemIconsHelper sharedInstance] iconNamed:CESystemIconProfileFontAndColor]];
    } else if ([[item itemIdentifier] isEqualToString:__fileTypesOptionsItemIdentifier]) {
      [item setImage:[[CESystemIconsHelper sharedInstance] iconNamed:CESystemIconMultipleItemsIcon]];
    } else if ([[item itemIdentifier] isEqualToString:__userInterfaceOptionsItemIdentifier]) {
      [item setImage:[[CESystemIconsHelper sharedInstance] iconNamed:CESystemIconSidebarPrefs]];
    }
  }
